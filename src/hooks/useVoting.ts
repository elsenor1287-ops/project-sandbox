...
import type { AppState, BallotOption, BallotSubmission, RCVResult, RCVRound, RankedVote } from '../types';
import { INITIAL_BALLOT_OPTIONS, MOCK_TEST_ACCOUNTS } from '../data/mockData';
...
export function useVoting(setState: Dispatch<SetStateAction<AppState>>) {
    ...
    setState(prev => {
      ...
      for (let i = 0; i < Math.min(count, accounts.length); i++) {
        ...
        if (!account.hasVoted) {
          ...
          const shuffled = [...prev.ballotOptions].sort(() => crypto.getRandomValues(new Uint32Array(1))[0] / (0xffffffff + 1) - 0.5);
          const rankings = shuffled.slice(0, Math.floor((crypto.getRandomValues(new Uint32Array(1))[0] / (0xffffffff + 1)) * 4) + 1).map((opt, idx) => ({
            optionId: opt.id,
            rank: idx + 1,
          }));

          // Randomly add a write-in (10% chance)
          const writeIn = (crypto.getRandomValues(new Uint32Array(1))[0] / (0xffffffff + 1)) < 0.1 ? `Citizen Initiative #${Math.floor((crypto.getRandomValues(new Uint32Array(1))[0] / (0xffffffff + 1)) * 100)}` : undefined;
