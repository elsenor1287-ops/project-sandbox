...
describe('useAppState', () => {
  ...
  describe('submitProposal', () => {
    it('compiles a valid proposal without any Law 1 violation keywords', () => {
      ...
      expect(stateProposal).toBeDefined();
      expect(stateProposal?.status).toBe('compiled');
    });

    it('vetoes a proposal containing a single Law 1 violation keyword', () => {
      const { result } = renderHook(() => useAppState());
      ...
      expect(newProposal.vetoReason).toBe('First Amendment Shield: "ban speech" detected');
      expect(newProposal.triggeredKeywords).toEqual(['First Amendment Shield: "ban speech" detected']);

      const stateProposal = result.current.state.proposals.find(p => p.id === newProposal.id);
      expect(stateProposal?.status).toBe('vetoed');
    });

    it('vetoes a proposal containing a Law 1 violation keyword with different casing', () => {
      const { result } = renderHook(() => useAppState());
    ...
    it('vetoes a proposal with multiple violation keywords and records all violations', () => {
      const { result } = renderHook(() => useAppState());

      let newProposal;
      act(() => {
        newProposal = result.current.submitProposal({
          title: 'Very Bad Proposal',
          content: 'This proposal will ban speech and confiscate guns from everyone.',
          tier: 'constitution',
          submittedBy: 'user-1',
        });
      });
      ...
      expect(newProposal.status).toBe('vetoed');
      expect(newProposal.vetoReason).toBe('First Amendment Shield: "ban speech" detected; Second Amendment Shield: "confiscate guns" detected');
      expect(newProposal.triggeredKeywords).toEqual([
        'First Amendment Shield: "ban speech" detected',
        'Second Amendment Shield: "confiscate guns" detected'
      ]);
    });
  });
